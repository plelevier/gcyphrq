...
**Scalar functions:** 25+ (`toLower`, `toUpper`, `substring`, `split`, `repl`, `trim`, `ltrim`, `rtrim`, `length`, `head`, `last`, `tail`, `reverse`, `size`, `id`, `labelsOf`, `reltype`, `startnode`, `endnode`, `coalesce`, `toString`, `toInteger`, `toFloat`). Work in RETURN/WHERE/WITH/ORDER BY, nested supported. Note: `repl`/`labelsOf`/`reltype` (not `replace`/`labels`/`type`) — ANTLR4 reserved. `startnode()`/`endnode()` return string IDs.
