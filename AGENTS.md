...
**Supported:** MATCH (labels/properties), OPTIONAL MATCH, variable-length paths `*min..max`, directional edges (`->`, `<-`, `-`), RETURN (property access, aliases), RETURN DISTINCT, WITH + implicit grouping, aggregations (`count`, `sum`, `avg`, `min`, `max`, `count(DISTINCT)`, `sum(DISTINCT)`, `avg(DISTINCT)`), WHERE (`>`, `<`, `=`, `<>`, `CONTAINS`, `STARTS WITH`, `ENDS WITH`, `IN`, `AND`/`OR`/`NOT`, IS NULL/IS NOT NULL, string comparisons for `<`/`>`), UNWIND, CREATE/SET/DELETE mutations, ORDER BY (multi-column, ASC/DESC), SKIP, LIMIT, multiple chained clauses (single MATCH per stage).
