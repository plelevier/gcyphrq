...
describe('Engine - MATCH', () => {
  ...
  describe('execute - MATCH', () => {
    it('finds all nodes matching a label', async () => {
      const ast = parseCypher('MATCH (u:User) RETURN u');
      const results = await engine.execute(ast);
      expect(results.length).toBe(4);
    });

    it('finds nodes matching a label and property', async () => {
    ...
    it('traverses inbound relationships', async () => {
    ...
    it('traverses inbound relationships from known node', async () => {
    ...
    it('traverses undirected relationships', async () => {
    ...
    it('traverses variable-length paths (min=1, max=2)', async () => {
    ...
    it('handles OPTIONAL MATCH with no match', async () => {
    ...
    it('handles OPTIONAL MATCH with a match', async () => {
  ...
  describe('execute - bare node pattern', () => {
    it('matches all nodes with bare pattern (no label)', async () => {
      const ast = parseCypher('MATCH (n) RETURN n');
      const results = await engine.execute(ast);
      expect(results.length).toBe(4);
    });

    it('matches all nodes with bare pattern and property filter', async () => {
  ...
  describe('execute - OPTIONAL MATCH edge cases', () => {
    it('preserves source variable in OPTIONAL MATCH with no match', async () => {
    ...
    it('nulls relationship variable in OPTIONAL MATCH with no match', async () => {
      const ast = parseCypher(
  ...
  describe('execute - diamond-graph DFS paths', () => {
    ...
    it('finds all three paths in a diamond graph with [*1..2]', async () => {
      const ast = parseCypher('MATCH (a:Node {name: "A"})-[r:LINK*1..2]->(d:Node {name: "D"}) RETURN a, d');
      const results = await diamondEngine.execute(ast);
      expect(results.length).toBe(3);
    });

    it('finds only the direct edge in diamond graph with [*1..1]', async () => {
    ...
    it('captures correct edge history for each diamond path', async () => {
  ...
  describe('execute - edge attributes', () => {
    it('returns edge attributes when relationship variable is bound', async () => {
      const ast = parseCypher(
  ...
  describe('execute - self-loops', () => {
    it('handles self-loop edges correctly', async () => {
    ...
    it('handles self-loop with variable-length path (cycle guard prevents depth 2)', async () => {
      const g = new Graph({ allowSelfLoops: true });
      g.addNode('a', { label: 'Node', name: 'A' });
      g.addEdge('a', 'a', { type: 'SELF' });
      const e = new AdvancedCypherGraphologyEngine(g);
